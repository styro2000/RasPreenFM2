...
void Voice::noteOn(short newNote, short velocity, unsigned int index) {
	...
	resetfxCoefficents();					// styro Polyfilter
...
//	float *sample = currentTimbre->getSampleBlock();			// styro
	float *sample = VoiceSampleBlock;
...
//	float inv32 = .03125f;							// Blocksize 16
	...
	float *TimbreSampleBlock = currentTimbre->getSampleBlock();
	for (int k =0; k< BLOCK_SIZE*2; k++) {
		TimbreSampleBlock[k] += VoiceSampleBlock[k];
	}

}

void Voice::applyFilter(void){
	//    float matrixFilterFrequency = 0.0f;
...
//    float gainTmp =  currentTimbre->getParamRaw()->effect.param3;
    ...
    switch (effectType) {
		case FILTER_LP:
		{
			float fxParamTmp = currentTimbre->getParamRaw()->effect.param1 + matrixFilterFrequency;
	...
	//    		Left voice
		...
		case FILTER_CRUSHER:
		{
			...
			float fxParamTmp = currentTimbre->getParamRaw()->effect.param1 + matrixFilterFrequency +.005f;
			...
			register float *sp = VoiceSampleBlock;
	...
	//        old1 = v;
			...
			float fxParam1PlusMatrixTmp = currentTimbre->getParamRaw()->effect.param1 + matrixFilterFrequency;
			...
			if (fxParam1PlusMatrix != fxParam1PlusMatrixTmp) {
				...
				recomputeBPValues(currentTimbre->getParamRaw()->effect.param2, fxParam1PlusMatrix * fxParam1PlusMatrix);
			}
		...
		case FILTER_LP2:
			{
				float fxParamTmp = LP2OFFSET + currentTimbre->getParamRaw()->effect.param1 + matrixFilterFrequency;
				fxParamTmp *= fxParamTmp;
		...
		case FILTER_BP2:
		{
			float fxParam1PlusMatrixTmp = clamp(currentTimbre->getParamRaw()->effect.param1 + matrixFilterFrequency, 0, 1);
			if (fxParam1PlusMatrix != fxParam1PlusMatrixTmp) {
				fxParam1PlusMatrix = fxParam1PlusMatrixTmp;
		        recomputeBPValues(currentTimbre->getParamRaw()->effect.param2, fxParam1PlusMatrix * fxParam1PlusMatrix);
			}
		...
		case FILTER_LP3:
		{
			...
			float fxParamTmp = SVFOFFSET + currentTimbre->getParamRaw()->effect.param1 + matrixFilterFrequency;
			fxParamTmp *= fxParamTmp;
			// Low pass... on the Frequency
			fxParam1 = clamp((fxParamTmp + 9.0f * fxParam1) * .1f, 0, 1);
		...
		case FILTER_LPHP:
		{
			...
			const int mixWet = (currentTimbre->getParamRaw()->effect.param1 * 122);
		...
		case FILTER_LPWS:
			{
				...
				const int mixWet = (currentTimbre->getParamRaw()->effect.param2 * 127);
		...
		case FILTER_TILT:
			{
				...
				const float gain = (currentTimbre->getParamRaw()->effect.param2 - 0.5f);
		...
		case FILTER_STEREO:
		{
			float fxParamTmp = fabsf(currentTimbre->getParamRaw()->effect.param1 + matrixFilterFrequency);
			fxParam1 = ((fxParamTmp + 19.0f * fxParam1) * .05f);

			float OffsetTmp = fabsf(currentTimbre->getParamRaw()->effect.param2);
		...
		case FILTER_ORYX:
			{
				...
				float param2Tmp = (currentTimbre->getParamRaw()->effect.param2);
...
// 			float coef1 = (1.0f - f1) / (1.0f + f1);
		...
		case FILTER_TEEBEE:
		{
			...
			bool isReleased = this->released;   // voices[this->lastPlayedNote]->isReleased();	
			bool accent = velocity > 0.629f && !isReleased;

//			const float ga = 0.9764716867f; //= exp(-1/(PREENFM_FREQUENCY * attack (0.028) / BLOCK_SIZE32))
			const float ga = 0.9881658194f; //= exp(-1/(PREENFM_FREQUENCY * attack (0.028) / BLOCK_SIZE16))
//			const float gr = 0.9979969962f; //= exp(-1/(PREENFM_FREQUENCY * release (0.3325) / BLOCK_SIZE32))
			const float gr = 0.9989979960f; //= exp(-1/(PREENFM_FREQUENCY * release (0.3325) / BLOCK_SIZE16))
...
// 	this->matrix.setSource(MATRIX_SOURCE_CC4,value);
	...
	setfxParam1(timbre->getParamRaw()->effect.param1);
	setfxParam2(timbre->getParamRaw()->effect.param2);
	setfxParam3(timbre->getParamRaw()->effect.param3);
	setfxParamA2(timbre->fxParamA2);
	setfxParamB1(timbre->fxParamB1);
	setfxParamB2(timbre->fxParamB2);
