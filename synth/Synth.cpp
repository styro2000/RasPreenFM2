...
//    float toAdd = 131071.0f*4;              // styro
    float toAdd = 0.0f;              // styro
...
//    for (int s = 0; s < 64/4; s++) {
    for (int s = 0; s < (BLOCK_SIZE*2)/4; s++) {
        ...
        // *cb++ = (int)((*sampleFromTimbre1++ + *sampleFromTimbre2++ + *sampleFromTimbre3++ + *sampleFromTimbre4++) + toAdd);
        // *cb++ = (int)((*sampleFromTimbre1++ + *sampleFromTimbre2++ + *sampleFromTimbre3++ + *sampleFromTimbre4++) + toAdd);
        *cb++ = (int)((*sampleFromTimbre1++ + *sampleFromTimbre2++ + *sampleFromTimbre3++ + *sampleFromTimbre4++));
        *cb++ = (int)((*sampleFromTimbre1++ + *sampleFromTimbre2++ + *sampleFromTimbre3++ + *sampleFromTimbre4++));
        *cb++ = (int)((*sampleFromTimbre1++ + *sampleFromTimbre2++ + *sampleFromTimbre3++ + *sampleFromTimbre4++));
        *cb++ = (int)((*sampleFromTimbre1++ + *sampleFromTimbre2++ + *sampleFromTimbre3++ + *sampleFromTimbre4++));
    }
...
void Synth::updateNumberOfActiveTimbres() {
    int activeTimbres = 0;
    if (synthState->fullState.midiConfigValue[MIDICONFIG_VOICELEVEL] == 1){
        ratioTimbre = ratiosTimbre[1];
    }else if  (synthState->fullState.midiConfigValue[MIDICONFIG_VOICELEVEL] == 2) {
        ratioTimbre = ratiosTimbre[2];
    }else if  (synthState->fullState.midiConfigValue[MIDICONFIG_VOICELEVEL] == 3) {
        ratioTimbre = ratiosTimbre[3];
    }else if  (synthState->fullState.midiConfigValue[MIDICONFIG_VOICELEVEL] == 4) {
        ratioTimbre = ratiosTimbre[3];        
    }else if  (synthState->fullState.midiConfigValue[MIDICONFIG_VOICELEVEL] == 5) {
        ratioTimbre = ratiosTimbre[3];        
    }else{
...
void Synth::newMenuSelect(FullState* fullState) {
    ...
    }else if  (fullState->currentMenuItem->menuState == MENU_CONFIG_SETTINGS                    // styro Outlevel-Config changed
            && fullState->menuSelect == MIDICONFIG_VOICELEVEL) {
            updateNumberOfActiveTimbres();
    }
#ifdef CVIN      
    if (fullState->currentMenuItem->menuState == MENU_CONFIG_SETTINGS) {
