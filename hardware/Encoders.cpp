...
// 	  enc1 IC2 In C&E Pin11&13 / enc2 IC2 In D IC1 In H Pin12&8 / enc3 IC1 In A IC1 In G Pin1&7 /  enc4 IC1 In B IC1 In F  Pin2&6  (nach schaltplan R5)

//    char buttonPins[] = { 9, 16, 10, 15, 11, 5, 4, 6};

// S1 MENU Pin5 / S2 INST Pin4 / S3 MOD Pin14 / S4 MTRX Pin10 / S5 OPParams Pin15 / S6 OP Pin9 / S7 ENG Pin16 / Encoderswitchs Pin3 (nach schaltplan R5)

//    char encoderPins[] = { 9,10,11,12,13,14,15,16};
    char encoderPins[] = { 16,15,14,13,12,11,10,9};
    char buttonPins[] = { 1,2,3,4,5,6,7,8};
	...
	*/
	// PIC11 ... N24
	// InPin.AssignPin(5);
	// InPin.SetMode(GPIOModeInputPullUp);
...
void Encoders::setButtonRegister( u16 *ButtonEncoderRegister){
	m_ButtonEncoderRegister = ButtonEncoderRegister;
}
...
int Encoders::getRegisterBits() {
	...
	u16 registerBits = *m_ButtonEncoderRegister;
	// if ((registerBits & 0x01) != 0){
	// 	out_led3.Set(1);	
	// }else{
	// 	out_led3.Set(0);					
	// }
...
//	for(int i=0; i<8; i++) {
		// registerBits |= (InPins[0].Read() << i) ;
//	}
//	registerBits |= (InPin.Read() << 0) ;
