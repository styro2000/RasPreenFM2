...
extern const struct MidiConfig midiConfig[];					//styro
...
void ConfigurationFile::fillMidiConfig(short* midiConfigBytes, char* line) {
	...
	for (int k=0; k < MIDICONFIG_SIZE; k++) {	
		...
		if (fsu->str_cmp(key, midiConfig[k].nameInFile) == 0) {
			...
			intvalue=0;
			intvalue=fsu->toInt(value);				//styro test on allowd value
			if (midiConfig[k].maxValue >=  intvalue && intvalue > 0){
				midiConfigBytes[k] = intvalue;
			}
			return;
		}
	}
